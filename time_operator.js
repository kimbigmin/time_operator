function showTime() {
  // DOM Setting
  const beforeTimeHour = document.getElementById("beforeTimeHour").value;
  const afterTimeHour = document.getElementById("afterTimeHour").value;
  const beforeTimeMin = parseInt(
    document.getElementById("beforeTimeMin").value
  );
  const afterTimeMin = parseInt(document.getElementById("afterTimeMin").value);
  ...
  if (differenceInt > 0) {
    document.getElementById(
      "showResult"
    ).innerHTML = `<p class="result_text">이전 시간보다 약 <span style="color:red;">${differenceInt}% <strong style="color:red;">증가</strong></span> 하였습니다!</p>`;

    document.getElementById(
      "newReset"
    ).innerHTML = `<button onclick="percentResetBtn()" class="reset">Reset</button>`;
  } else if (differenceInt <= 0) {
    document.getElementById(
      "showResult"
    ).innerHTML = `<p class="result_text">이전 시간보다 약 <span style="color:blue;">${differenceInt}% <strong style="color:blue;">감소</strong></span> 하였습니다.</p>`;
    ...
    document.getElementById(
      "showResult"
    ).innerHTML = `<p class="result_text" style="color:#F6A000; font-weight:bold; margin-left:3rem;">시간이 입력되지 않았습니다. 시간을 정확히 입력해주세요.</p>`;
...
function getAverageTime() {
  ...
  if (isNaN(resultHour) && isNaN(resultMin)) {
    document.getElementById(
      "showResult"
    ).innerHTML = `<p class="result_text_avg" id="avgText" style="color:#F6A000; margin-left: 6rem; font-weight:bold;">시간이 입력되지 않았습니다. 시간을 정확히 입력해주세요.</p>`;

    document.getElementById(
      "newReset"
    ).innerHTML = `<button onclick="avgResetBtn()" class="reset" id="avgBtn">Reset</button>`;
  } else {
    document.getElementById(
      "showResult"
    ).innerHTML = `<p class="result_text_avg" id="avgText">평균 취침 & 기상시간은 <strong>${resultHour}시 ${resultMin}분</strong> 입니다.</p>`;
...
function getSumTime() {
  ...
  if (isNaN(resultHour) && isNaN(resultMin)) {
    ...
    document.querySelector(
      "#showResult"
    ).innerHTML = `<p class="result_text_avg" id="avgText">총합 시간은 <strong>${resultHour}시간 ${resultMin}분</strong> 입니다.</p>`;
...
const openMenubar = function () {
  ...
  percentPage.innerHTML =
    '<a href="./time_operator.html">시간 퍼센트 구하기</a>';
  ...
  weeklyPage.innerHTML =
    '<a href="./week_operator.html">일주일 시간합계 구하기</a>';
